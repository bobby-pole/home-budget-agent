...
pnpm build         # Production build (runs generate:types + tsc first)
pnpm lint          # ESLint
...
docker-compose exec backend pytest tests/test_lidl_parser.py    # single file
docker-compose exec backend pytest -k "test_name"               # single test
docker-compose exec backend ruff check app/                      # linting
docker-compose exec backend pyright                              # type checking
```

### Contract-first API workflow

`contracts/openapi.json` is the source of truth for all API types. After any change to FastAPI endpoints or response models, regenerate the frontend client:

```bash
# 1. Export updated schema from FastAPI
...
`frontend/src/client/` is **auto-generated** — never edit it manually.
...
- `backend/app/ocr_pipeline.py` — OCR pipeline, line reconstruction from bounding boxes, `ReceiptSource` routing
- `backend/app/lidl_parser.py` — Lidl deterministic parser (state machine + regex); extend here for new Lidl formats
- `backend/app/receipt_validator.py` — sum validation (items total == receipt total ±0.01 PLN)
- `contracts/openapi.json` — API contract; generated by `export_openapi.py`, consumed by frontend `generate:types`
...
- `frontend/src/client/` — auto-generated API client from OpenAPI (**do not edit manually**)
- `frontend/src/locales/en.json` + `pl.json` — i18n strings; **always update both** when adding UI text
...
- **i18n namespaces**: `receipt_status`, `auth`, `nav`, `dashboard`, `inbox`, `transactions`, `budget`, `settings`, `upload`, `quick_entry`, `monthly_summary`, `spending_chart`, `common`, `tags`, `tag_picker` — add new keys under the matching namespace in both locale files
