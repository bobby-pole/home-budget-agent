...
describe("BudgetSummaryCard", () => {
  ...
  it("renders budget figures correctly", () => {
    ...
    expect(screen.getByText(/z 5.*000.*PLN zaplanowanych/i)).toBeInTheDocument();
  ...
  it("shows empty state CTA when no budget is set", () => {
    render(<BudgetSummaryCard {...defaultProps} planned={0} spent={0} />);
    
    expect(screen.getByText(/Nie masz jeszcze budżetu/i)).toBeInTheDocument();
    const btn = screen.getByRole("button", { name: /Skonfiguruj budżet/i });
